...
class VerificationService {
    ...
    static async verifyAndSettle(taskId, provenanceHash, result) {
        ...
        // 3. Settlement logic
        const treasury = process.env.TREASURY_ADDRESS || contracts_1.adminAddress;
        ...
        if (kiteResult.valid) {
            ...
            try {
                ...
                if (task.agent?.address) {
                    await contracts_1.registryClient.send.incrementTasks({
                ...
                await prisma_1.prisma.task.update({
                    where: { id: taskId },
                    data: { state: types_1.TaskState.VERIFIED }
                });
            }
        }
        else {
            ...
            try {
                ...
                if (task.agent?.address) {
                    await contracts_1.registryClient.send.incrementTasksFailed({
                        args: { agentId: task.agentId }
                    });
                }
